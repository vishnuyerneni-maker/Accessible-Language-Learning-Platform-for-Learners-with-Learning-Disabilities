    ...
    return (
        <div className="game-overlay animate-fade-in">
            <div className="card game-modal" style={{ maxWidth: '900px', width: '90%', textAlign: 'center', padding: '20px' }}>
                ...
                <h2 style={{ marginBottom: '20px' }}>✍️ Trace Letter {letter}</h2>
                ...
                <div style={{ display: 'flex', gap: '15px', justifyContent: 'center', marginTop: '20px' }}>
                    <button onClick={clearCanvas} className="btn btn-outline">🗑️ Clear</button>
                    <button onClick={() => {
                        const utterance = new SpeechSynthesisUtterance(letter);
                        window.speechSynthesis.speak(utterance);
                    }} className="btn btn-primary">🔊 Hear Letter</button>
                </div>
            </div>
        </div>
    );
